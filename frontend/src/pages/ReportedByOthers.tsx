...
export default function ReportedByOthers() {
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f5] flex flex-col">
      <header className="bg-white border-b border-gray-100 flex items-center justify-between px-4 py-3.5 sticky top-0 z-10">
        ...
        <h1 className="font-heading font-bold text-black text-base">Reported by others</h1>
        <HamburgerButton />
      </header>
      <main className="flex-1 p-4 space-y-3 max-w-[430px] mx-auto w-full">
        ...
        {loading ? (
          ...
          <p className="text-gray-500 text-sm text-center py-12">No incidents reported by others yet.</p>
        ) : (
          reports.map((report) => (
            <Link
              key={report.id}
